...
- **阶段**: 架构设计完成，优先级已优化
- **状态**: ✅ 架构设计完成，architect agent 审查通过

---

## 🔄 v1.2 更新（architect agent 优先级审查）

### 审查结果摘要

**审查日期**: 2026-01-31
**审查人**: AI architect agent
**审查范围**: ADR-002/003/004 任务优先级
...
| 模块 | 方法/算法 | 工作量 | 变化 |
|------|----------|--------|------|
| **标准化方法** | MinMax + Vector | 1.5 人日 | ✅ 保持 |
| **赋权方法** | 直接赋权（手动指定）| 0.5 人日 | ⭐ 新增 MVP 核心 |
| **汇总算法** | WSM + WPM + TOPSIS + **VIKOR** | 5 人日 | ⭐ VIKOR 升级到 v0.2 |
| **测试与文档** | 单元测试 + 使用文档 | 3 人日 | - |

**关键变化**:
- ✅ **VIKOR 升级到 v0.2 (P0)**: 唯一提供折衷解的算法，符合"权衡取舍"核心需求
- ✅ **直接赋权优先**: 用户在 YAML 中直接指定权重（最常见场景，80% 用户）
- ⬇️ **赋权方法降级**: 熵权法、变异系数法等从 v0.2 降级到 v0.3/v0.4
- ⬇️ **SAW 大幅降级**: 与 WSM 几乎无差异，从 v0.2 降级到 v0.4

#### 详细优先级调整

**标准化方法（ADR-002）**:
- Vector 升级：架构依赖从 4.5 → 5.0（TOPSIS 必需）
- Inverse 降级：可作为 MinMax 的 direction 参数实现
- Threshold/Max 降级：丢失精度/功能重复

**赋权方法（ADR-003）**:
- **新增直接赋权（P0, v0.2）**: 用户手动指定权重
- AHP 升级：从 v0.4 → v0.3（最热门主观方法）
- 熵权法降级：从 v0.2 → v0.3（客观赋权非 MVP 核心）
- 变异系数法/标准离差法降级：功能重复

**汇总算法（ADR-004）**:
- **VIKOR 升级到 v0.2 (P0)**: 唯一提供折衷解，用户价值从 4.5 → 5.0
- TOPSIS 评分升级：架构依赖从 4.5 → 5.0（Vector 标准化必需）
- SAW 大幅降级：与 WSM 几乎无差异，兼容性从 5.0 → 2.0

### 优化理由

1. **用户价值导向**: MVP 聚焦用户核心需求（决策 + 折衷）
2. **快速验证**: 2 周交付可用版本，获得用户反馈
3. **工作量优化**: 节省 16 人日（17%）
4. **风险可控**: 保留回退方案
...
- **创建时间**: 2026-01-31
- **最后更新**: 2026-01-31
- **文档版本**: v1.2 (architect agent 优先级审查)
- **状态**: ✅ 架构设计完成，优先级已优化
...
**预期工作量（architect agent 审查后调整）**:
- v0.2 (MVP): 10 人日（2 周）⭐
- v0.3: 19 人日（4 周）
- v0.4: 31 人日（6 周）
- v0.5: 20.5 人日（4 周）

**总工作量**: 80.5 人日 (原 96.5 人日，优化节省 16 人日)
