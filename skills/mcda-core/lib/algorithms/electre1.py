...
def _build_rankings(
    ...
    for alt in kernel:
        ...
        kernel_scores.append((alt, score))

    # 按得分降序排列
    kernel_scores.sort(key=lambda x: x[1], reverse=True)
    ...
    non_kernel_scores = []
    for alt in non_kernel:
        idx = alternatives.index(alt)
        score = np.sum(credibility[idx, :])
        non_kernel_scores.append((alt, score))

    # 按得分降序排列
    non_kernel_scores.sort(key=lambda x: x[1], reverse=True)
